...
const inputLoss = document.getElementById("loss");
...
async function getSolarData() {
  ...
  const loss = inputLoss.value;

  // PVGIS API v5.3
  const targetUrl = `https://re.jrc.ec.europa.eu/api/v5_3/PVcalc?lat=${selectedLat}&lon=${selectedLon}&peakpower=${peakPower}&loss=${loss}&angle=${angle}&aspect=${aspect}&outputformat=json`;
  ...
  if (!response || !response.ok) {
    throw new Error(
      `All proxies failed. Last error: ${lastError ? lastError.message : "Unknown"}`,
    );
  }
